  ...
  el.querySelector('#btn-confirm').addEventListener('click', () => {
    ...
    rem.innerHTML = `
      <div class="d-flex gap-2 mb-2">
        <button class="btn btn-sm btn-outline-secondary" id="btn-print-rem"><i class="bi bi-printer"></i> Imprimir</button>
        <button class="btn btn-sm btn-primary" id="btn-pdf-rem"><i class="bi bi-filetype-pdf"></i> PDF</button>
      </div>
      <div class="print-area mt-3 border rounded p-3">
        <div class="text-center mb-2">
          <img data-logo src="./public/assets/logo.svg" alt="SCANIX – Sistema de Reconocimiento de Productos" style="max-height:80px"/>
    ...
    // Ensure logo matches current theme when rendering remito
...
function downloadRemitoPDF(remito, depots) {
  ...
  try {
    ...
    const img = new Image();
    img.onload = () => {
      const logoHeight = 14, logoWidth = 14; const x = (pageWidth - logoWidth)/2;
      ...
      remito.lines.forEach(l => { doc.text(l.sku, 10, y); doc.text(l.name, 40, y); doc.text(String(l.qty), 200, y, { align: 'right' }); y+=6; });
...
export function TransfersPage() {
  ...
  el.addEventListener('click', (e) => {
    ...
    const inc = e.target.closest('[data-inc]');
    const rem = e.target.closest('[data-remove]');
    if (dec) { const id = dec.getAttribute('data-dec'); const l = lines.find(x=>String(x.productId)===String(id)); if (l) { l.qty = Math.max(0, l.qty-1); render(); } }
    if (inc) { const id = inc.getAttribute('data-inc'); const l = lines.find(x=>String(x.productId)===String(id)); if (l) { l.qty += 1; render(); } }
    if (rem) { const id = rem.getAttribute('data-remove'); lines = lines.filter(x=>String(x.productId)!==String(id)); render(); }
  });
  q.addEventListener('input', () => runSearch(q.value));
  q.addEventListener('keydown', (e) => {
...
function downloadRemitoPDF(remito, depots) {
  ...
  try {
    ...
    const img = new Image();
    let done = false;
    const safeRender = () => { if (done) return; done = true; renderBody(); };
    img.onload = () => { try { const s=14; const x=(pageWidth - s)/2; doc.addImage(img, 'PNG', x, y, s, s, undefined, 'FAST'); y += s + 4; } catch(_) {} finally { safeRender(); } };
